...
@WebServlet(name="p_1_2", urlPatterns={"/p_1_2"})
public class p_1_2 extends HttpServlet {
  ...
  static final String nazwabazy="sklep_online_gsm";
   ...
   String d = request.getParameter("d");
    ...
    HttpSession session=request.getSession();
      ...
      Statement sql; Statement sql0150; Statement sql090; Statement sql15;  
...
"</div>");   
      ...
      ResultSet rs9 = sql9.executeQuery("SELECT DISTINCT user.imie,user.nazwisko,user.miasto,user.ulica,user.nr_u,user.kod,user.telefon,user.email from user,koszyk where koszyk.id_user='"+id_u+"' and koszyk.data_realizacji='"+d+"' and koszyk.realizacja='1' and koszyk.id_user=user.id_user;");
  while(rs9.next())  {


            out.print("<h2><strong>Klient</strong>: ");
                 out.print(""+rs9.getString(1)+" "+rs9.getString(2)+"</h2>");

        out.print("<h3><strong>Dane adresowe:</strong> ");
        out.print(""+rs9.getString(3)+" <b>&nbsp;ul. &nbsp;</b>"+rs9.getString(4)+" <b>numer </b>"+rs9.getString(5)+"<b> &nbsp;kod&nbsp;</b> "+rs9.getString(6)+"</h3>");
          out.print("<h3><strong>Dane kontaktowe:&nbsp;</strong> ");
        out.print("<b>&nbsp;tel. </b>&nbsp;"+rs9.getString(7)+" &nbsp;<b>&nbsp;email: &nbsp;</b>"+rs9.getString(8)+" </h3>");
...
"              <tbody>");
      ...
      ResultSet rs15 = sql15.executeQuery("Select magazyn.nazwa, magazyn.cena, magazyn.id_magazyn,koszyk.id_koszyk,koszyk.ilosc,magazyn.obraz from magazyn,koszyk,opis_towaru where koszyk.id_user='"+id_u+"' and koszyk.data_realizacji='"+d+"'  and koszyk.id_magazyn=magazyn.id_magazyn and magazyn.id_magazyn=opis_towaru.id_magazyn;");
int nr_wiersza=1;
float koniec=0;
      while(rs15.next())  {
...
koniec += sumka;
         ...
         out.println(" <tr>");
            out.println("<td ><td></td><td></td><td><strong>SUMA</strong></td>");
           out.println(" <td class=\"label label-important\" style=\"display:block\"> <strong><font color='gree'><b>" + koniec);

                 out.println(" z&#322;</b></font></strong></td></tr>");
       ...
       out.println(" </table>");
          ...
          out.write("	<a href=\"p_zamowienie3?dd="+request.getParameter("d")+"&id_uu="+request.getParameter("id_u")+"&id_user="+request.getParameter("id_user")+"\" class=\"btn btn-large pull-right\">Zamknij zlecenie<i class=\"icon-arrow-right\"></i></a>");
