...
function przetwarzajj_dane (){
 ...
 var WzorMaila = /^[0-9a-z_.-]+@[0-9a-z.-]+\.[a-z]{2,3}$/i
  ...
  {
    napis += "Niepoprawnie wprowadzony Adres email !\n"
    brakuje_danych = true;
  }
...
<div class="menu pst-top-menu col-md-12 col-lg-12  js-top-menu position-static hidden-md-down" id="_desktop_top_menu">	
 ...
 <li class="category current " id="category-9"> <a class="dropdown-item" href="logowanie.jsp" data-depth="0" > Logowanie </a></li>
...
<div id="_desktop_cart">
  <div class="blockcart cart-preview inactive" data-refresh-url="module/ps_shoppingcart/ajax">
    <div class="header blockcart-header dropdown js-dropdown">
			...
			<a rel="nofollow" href="#" class="cart-icon"  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> 							
...
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
  ...
  <li class="category"><a href="kategorie3?id_kategorie=<%= rs2.getString(2)%>&id_user=<%= session.getAttribute("id_user") %>" class="dropdown-item" data-depth="0"><%= rs2.getString(1)%> </a></li>
	...
	<!--<h4 class="title_block">Search</h4>-->
		...
		<div class="block_content clearfix">
			...
			<input class="search_query form-control grey" type="text" name="fraza" value="" placeholder="Szukaj produktu.." />
			 ...
			 <input  type='hidden' name='id_user' size="30" value="<%= session.getAttribute("id_user") %>">
	  ...
	  <section id="wrapper">
          ...
          <div id="columns_inner">
            ...
            <div id="left-column" class="col-xs-12 col-sm-4 col-md-3 col-lg-3">
                ...
                <p><b>Witamy<strong>  <font color="#990000"><%= session.getAttribute("imie") %> <%= session.getAttribute("nazwisko") %></font> - pracownik sklepu</strong></b></p>
...
<li ><a href="p1?id_user=<%= session.getAttribute("id_user") %>">Przegląd zamówień</a></li>
<li ><a href="a_dodaj?id_user=<%= session.getAttribute("id_user") %>">Dodaj produkt</a></li>
<li ><a href="p_fakt_klien?id_user=<%= session.getAttribute("id_user") %>">Faktury klientów</a></li>
<li ><a href="p_dane_klient?id_user=<%= session.getAttribute("id_user") %>">Dane użytkowników</a></li>
<li ><a href="p_wyszukiwarka?id_user=<%= session.getAttribute("id_user") %>">Wyszukiwarka</a></li>
<li ><a href="p_producenci?id_user=<%= session.getAttribute("id_user") %>">Producenci</a></li>
<li ><a href="p_kategorie?id_user=<%= session.getAttribute("id_user") %>">Kategorie</a></li>
<li ><a  href="p_zmina?id_user=<%= session.getAttribute("id_user") %>">Zmiana Hasla</a></li>
<li ><a href="logaut">Wyloguj</a></li>
...
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
  ...
  <li ><a href="kategorie3?id_kategorie=<%= rs2.getString(2)%>&id_user=<%= session.getAttribute("id_user") %>" ><%= rs2.getString(1)%> </a></li>
...
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
    while(rs2.next()) {
    %>
      <li><a href="producent3?id_producent=<%= rs2.getString(2)%>&id_user=<%= session.getAttribute("id_user") %>"><%= rs2.getString(1)%> </a></li>
