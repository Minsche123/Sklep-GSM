...
<script type="text/javascript">
  ...
  <body id="category" class="lang-en country-us currency-eur layout-left-column page-category tax-display-disabled category-id-20 category-gaming-console category-id-parent-2 category-depth-level-2">
<main>
<header id="header">
...
<div class="menu pst-top-menu col-md-12 col-lg-12  js-top-menu position-static hidden-md-down" id="_desktop_top_menu">	
 ...
 <li class="category" id="category-20"><a class="dropdown-item" href="rejestracja.jsp" data-depth="0"> Rejestracja</a> </li>
...
<div class="vertical-menu js-top-menu position-static hidden-md-down"  id="_desktop_vertical_menu">
  ...
  try {
    String url = "jdbc:mysql://localhost/sklep_online_gsm";
   Connection con = DriverManager.getConnection(url,"root","krasnal");
    Statement stmt = con.createStatement();
    String retrieveString = "select nazwa,id_rodzaj from rodzaj order by nazwa;";
    ResultSet rs = stmt.executeQuery(retrieveString);
                         if (rs.next())
                            {
Statement stmt2 = con.createStatement();
 String retrieveString2 = "select nazwa,id_rodzaj from rodzaj order by nazwa;";
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
  ...
  <li class="category"><a href="kategorie?id_kategorie=<%= rs2.getString(2)%>" class="dropdown-item" data-depth="0"><%= rs2.getString(1)%> </a></li>
...
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
  ...
  <li ><a href="kategorie?id_kategorie=<%= rs2.getString(2)%>" ><%= rs2.getString(1)%> </a></li>
...
<div class="sidebar-featured block">
 ...
 <div class="block_content collapse" id="block_featured_toggle"> 
  ...
  try {
   ...
   Connection con = DriverManager.getConnection(url,"root","krasnal");
    Statement stmt = con.createStatement();
    String retrieveString = "select typ,id_typ from typ order by typ;";
    ResultSet rs = stmt.executeQuery(retrieveString);
                         if (rs.next())
                            {
Statement stmt2 = con.createStatement();
 String retrieveString2 = "select typ,id_typ from typ order by typ;";
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
    while(rs2.next()) {
    %>
    <li><a href="producent?id_producent=<%= rs2.getString(2)%>"><%= rs2.getString(1)%> </a></li>
...
%>
  ...
  try {
   ...
   Connection con = DriverManager.getConnection(url,"root","krasnal");
    Statement stmt = con.createStatement();
    String retrieveString = "Select rodzaj.nazwa,magazyn.cena,magazyn.nazwa,magazyn.obraz,magazyn.id_magazyn,magazyn.id_magazyn,rodzaj.nazwa,rodzaj.nazwa,opis_towaru.opis from magazyn,rodzaj,opis_towaru where magazyn.aktywny='1' and magazyn.id_rodzaj=rodzaj.id_rodzaj  and magazyn.id_magazyn=opis_towaru.id_magazyn and magazyn.id_rodzaj=rodzaj.id_rodzaj ORDER BY RAND() LIMIT 8;";
    ResultSet rs = stmt.executeQuery(retrieveString);
                         if (rs.next())
                            {
Statement stmt2 = con.createStatement();
 String retrieveString2 = "Select rodzaj.nazwa,magazyn.cena,magazyn.nazwa,magazyn.obraz,magazyn.id_magazyn,magazyn.id_magazyn,rodzaj.nazwa,rodzaj.nazwa,opis_towaru.opis from magazyn,rodzaj,opis_towaru where magazyn.aktywny='1' and magazyn.id_rodzaj=rodzaj.id_rodzaj and magazyn.id_magazyn=opis_towaru.id_magazyn and magazyn.id_rodzaj=rodzaj.id_rodzaj ORDER BY RAND() LIMIT 8;";
ResultSet rs2 = stmt2.executeQuery(retrieveString2);
   while(rs2.next()) {

                   String myString = ""+rs2.getString(9)+"";
 String d = myString.substring(0, 100);
...
<div class="product-miniature js-product-miniature" data-id-product="19" data-id-product-attribute="0" itemscope itemtype="http://schema.org/Product">
  <div class="thumbnail-container">
    <div class="image-block">
      ...
      <a href="k_1_345?id_magazyn=<%= rs2.getString(6)%>" class="thumbnail product-thumbnail">
<img class = "primary-image" src = "<%= rs2.getString(4)%>">
<img class="replace-2x img_hover img-responsive secondary-image" src="<%= rs2.getString(4)%>" 
 data-full-size-image-url="<%= rs2.getString(4)%>" alt="" />
</a>
	  
        <div class="product-actions">
            <a href="k_1_345?id_magazyn=<%= rs2.getString(6)%>" class="btn btn-primary view-button" title="View Detail">                Zobacz więcej</a>
...
<a href="k_1_345?id_magazyn=<%= rs2.getString(6)%>" title="consectetur adipiscing" title="consectetur adipiscing"><%= rs2.getString(3)%></a></h3>
<div class="product-price-and-shipping">
<span itemprop="price" class="price"><%= rs2.getString(2)%> zł</span>
...
<div class="product-detail" itemprop="description"><p>"<%= rs2.getString(9)%>"</p></div>
